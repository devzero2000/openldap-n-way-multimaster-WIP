...
CHECK_SCRIPT="./check_direct_sync.sh"
...
while [ "$ELAPSED" -lt "$TIMEOUT" ]; do
    if "$CHECK_SCRIPT" > "$TEMP_LOG" 2>&1; then
        ...
        printf -- "\n[SUCCESSO] Cluster completamente allineato in %d secondi!\n" "$ELAPSED"
        printf -- "---------------------------------------------------------------------\n"
        cat "$TEMP_LOG"
        exit 0
    else
...
printf -- "\n\n[TIMEOUT] Il cluster NON si è allineato entro il limite di %d secondi.\n" "$TIMEOUT"
cat "$TEMP_LOG"
exit 1
