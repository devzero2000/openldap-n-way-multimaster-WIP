...
printf -- "\n*************** VERIFICA REPLICHE READONLY ****************\n"

printf -- "\n--- VERIFICA DIRETTA NODO 4 <-- NODO 1 ---\n"
# CORRETTO: Cambiato da -I "002" a -I "001" per tracciare sempre la propagazione del Nodo 1
if "$SCRIPT_PERL" -H "ldaps://ubuntu24lts4.example.com" -U "ldaps://ubuntu24lts1.example.com" -D "$ADMIN_DN" -P "$ADMIN_PW" -S "$BASE_DN" -I "001" -w 10 -c 15 > /dev/null 2>&1; then
    printf -- "[OK] Nodo 4 è in sync con Nodo 1.\n"
else
    printf -- "[ERR] Nodo 4 NON è in sync con Nodo 1.\n"
    CLUSTER_OK=1
fi
